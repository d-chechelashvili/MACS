...
PTRS = [PTR_BASE, PTR_GAP1, PTR_GAP2]
gradients = [(-1, -1), (0, -1), (-1, 0)]


def update_cell(F, TB, i, j, k, seq1, seq2, subst_matrix, gap_pen, max_score):
    di, dj = gradients[k]
    curr_score = F[i + di][j + dj] - gap_pen
    if k == 0:
        curr_score = F[i + di][j + dj] + subst_matrix[base_idx[seq1[i + di]]][base_idx[seq2[j + dj]]]
...
def seqalignDP(seq1, seq2, subst_matrix, gap_pen):
    ...
    for i in xrange(1, len(seq1) + 1):
        for j in xrange(1, len(seq2) + 1):
            max_score = -float('inf')
            for k in xrange(0, 3):
                max_score = update_cell(F, TB, i, j, k, seq1, seq2, subst_matrix, gap_pen, max_score)
...
def get_distance(seq1, seq2, S, gap_pen, score):
    """returns the distance between seq1 and seq2"""
    # დისტანცია წარმოვიდიგნოთ როგორც
...
def main():
    ...
    print "Distance:", get_distance(seq1, seq2, S, gap_pen, score)
